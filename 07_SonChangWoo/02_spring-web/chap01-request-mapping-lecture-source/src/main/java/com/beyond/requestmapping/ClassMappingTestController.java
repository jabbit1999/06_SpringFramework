package com.beyond.requestmapping;
...
public class ClassMappingTestController {
  ...
  public String postRegist(Model model) {
    model.addAttribute(
        "message", "GET 방식의 주문 등록 핸들러 메서드 호출");
  ...
  public String postModifyAndDeleteOrder(Model model) {
    model.addAttribute(
        "message", "POST 방식의 주문 수정 또는 삭제 핸들러 메서드 호출");
...
/*  @PostMapping("/delete")
  public String postDelete(Model model) {
    model.addAttribute(
        "message", "POST 방식의 주문 삭제 핸들러 메서드 호출");
  ...
  * */
  ...
  public String otherRequest(Model model) {
    ...
    model.addAttribute(
        "message", "order 요청이긴 하지만 다른 기능은 준비되지 않음");
